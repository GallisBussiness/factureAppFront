...
function UpdateFactureVenteModal({ isOpen, onResolve, onReject,client,factureVente }) {
 
    const defaultValues = {date: formatISO(new Date(),{ representation: 'date' }), client,avance: factureVente?.avance, total: factureVente?.total};
  ...
  const onCreate = data => {
       ...
       const avi = +avance;
       const fd = {_id: factureVente?._id,...data,total: +total,avance: avi, restant: +total - avi}
      onResolve(fd);
    };

  return (
    <>
        <Dialog header="Enregistrer Dette Client" visible={isOpen} onHide={() => onReject(false)} className="w-2/3">
