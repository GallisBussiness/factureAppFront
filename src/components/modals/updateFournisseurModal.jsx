...
function UpdateFournisseurModal({ isOpen, onResolve, onReject,fournisseur }) {
  ...
  const defaultValues = {nom: fournisseur?.nom, prenom: fournisseur?.prenom,tel: fournisseur?.tel};
...
const onCreate = data => {
  ...
  return (
    <>
      <Dialog header="Mettre à jour un fournisseur" visible={isOpen} onHide={() => onReject(false)} className="w-1/2">
